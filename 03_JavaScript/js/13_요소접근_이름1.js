...
function checkbox로 valueChecked속성확인(){
    //name의 속성값이 "취미"인 요소를 모두 얻어와 hobbys에 저장
    //보통 radio나 checkbox는 두 가지 이상의 input을 작성하고 취미나 마케팅 수신동의, 결혼여부처럼 각 input을 묶어주는 역할을 하는 것이 name
    //name은 최소 2가지 이상을 가져와서 사용
    const hobbys = document.getElementsByName("취미");
    /*alert("1 번 hobbys만 작성 : " + hobbys);//object NodeList 유사한 배열형태다
    alert("2번 hobbys[0] 만 작성 : " + hobbys[0]);//object HTMLInputElement html input
    alert("2번 hobbys[0].value 모두 작성 : "+ hobbys[0].value);//유사배열에서 0번째에*/

    //취미라는 것을 모두 가져와서 확인할 수 있음
    for(let i = 0; i< hobbys.length; i++){
        // hobbys 취미들은 현재 게임과 음악감상밖에 없으므로 index = 0, 1 만 존재 length는 2의 값을 가짐
...
function nameBtn(){
    ...
    for(let i = 0; i < hobbyList.length; i++){
        //for문으로 i를 순회할 때 i 값이 true인 경우만 count 추가
        if(hobbyList[i].checked){
         ...
         //document.getElementById("result").innerText = `${str} <br>선택된 취미 개수 : ${count}`
