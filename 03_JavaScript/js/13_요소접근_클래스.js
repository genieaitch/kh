...
function clsTest(){
    ...
    index = [] 0번째부터 length-1번째까지 번호를 매기는 것/ 자동으로 번호가 매겨짐

    length는 = [] 안에 값이 얼마나 들어있는지 1부터 개수를 세는 것
    */

    const divs = document.getElementsByClassName("cls");
    // 유사배열( = HTMLCOllection = html 묶음들)
    alert(divs); //유사배열(=HTMLCOll3ction = html("cls");

    // divs = cls 중에서 0번째로 작성된 div요소 배경색 변경
    ...
    //index는 무조건 0부터 시작하기 때문에 let i = 0 에서 시작하는 것
