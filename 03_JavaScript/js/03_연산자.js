...
function 더하기() {
  // input 태그에 작성된 값 = value 값 = value = 라틴어 의미 가치
  숫자1.value;
  숫자2.value;
  //숫자1.값과 숫자2.값을 더해줌

  /*
  →input에서 type을 number로 설정해줘도 input태그 안에서 작성된 값은 무조건 글자형식으로 인식함
  ...
  → 컴퓨터 용어 데이터 파싱(data parsing) : 데이터의 종류를 바꿈
    ...
    예) 123이 있다면 Number(123) 작성하면 됨
    밑에서는 Number(숫자1.value);
  */

  /*두 수의 합을 아이디가 계산결과인 요소에 내부글자(innerText)로 넣어주기*/
  계산결과.innerText = Number(숫자1.value) + Number(숫자2.value);
}
...
/*나머지 연산 함수
나머지 연산(%)는 나누기 계산시 몫이 정수인 부분까지만 계산하고 남은 나머지 값을 출력*/
function 나머지() {
  const value1 = 숫자1.value;
  const value2 = 숫자2.value;
  결과.innerText = value1 % value2;
