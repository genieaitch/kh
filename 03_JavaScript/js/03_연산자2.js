...
count라는 변수를 사용해서 계산을 누적하고 결과만 result 표시하는 방식
작성을 하지 않아도 동작을 하긴 하 지만 누적된 값이 사라지거나 변환될 수 있기 때문에
누적된 값을 저장하는 공간을 따로 설정해서 안전하게 보관하는 역할
*/

function plus(){
    /*result.innerText = 글자형식으로 취급 될 수 있기 떄문에 동일하게 Number로 감싸서 숫자+숫자끼리 누적이 될 수 있도록 설정*/
    const vl = Number(input.value); //이미 숫자로 모든 누적처리를 끝낸 상태
    count += vl
    //글자 += 숫자 + 숫자로 변환한다고 하지만 글자 뒤에 숫자를 넣는 것에 불과할 수 있음
    //누군가는 숫자로 모두 처리가 되어서 누적해서 숫자가 올라갈 수 있음
    result.innerText = count;
}
