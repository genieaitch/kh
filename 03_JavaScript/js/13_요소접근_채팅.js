function readValue(){
    ...
    //id를 작성하더라도 css에서 작성하는 아이디 표기 방법을 사용 ex) #아이디속성명
    ...
    *****문자열.trim() : 문자열의 좌우 공백을 제거*****
    ...
    inputChat.value → input 채팅 내용하는 창 안에 들어있는 값의
    trim()          → 좌우 공백을 제거하고
    length          → 총 글자수가
    inputChat.value.trim().length == 0) → 0일 때
    */

    if(inputChat.value.trim().length == 0){//입력한 게 없거나 띄어쓰기만 보내려한다면
        alert("채팅 내용을 입력해주세요.") //채팅내용 입력해서 보내기

        //채팅내용을 입력 하지 않았으면 돌려보내기
        return;
    }
     ...
     */
    bg.scrollTop = bg.scrollHeight; // 전체 div 높이를 가져옴으로써 스크롤을 맨 아래로 내려버리는 트릭
    inputChat.value="";
}
...
올라온 키가 'Enter'키 라면 readValue() 함수 불러오기
*/
